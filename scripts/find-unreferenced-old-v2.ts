...
public runtimeReachable = (entryFiles: Set<string>): Set<string> => {
  try {
    ...
    const configFile = ts.readConfigFile(tsconfigPath, ts.sys.readFile);
    if (configFile.error) return new Set<string>();

    const parsed = ts.parseJsonConfigFileContent(configFile.config, ts.sys, path.dirname(tsconfigPath));
    if (parsed.errors.length > 0) return new Set<string>();
    ...
    for (const sourceFile of program.getSourceFiles()) {
      ...
      ts.forEachChild(sourceFile, node => {
        ...
        // 注意：即使是 import type，如果类型在代码中被实际使用，也应该计入依赖
        // 我们通过后续的 AST 扫描来判断是否真的被使用
        ...
        if (ic.namedBindings) {
          if (ts.isNamespaceImport(ic.namedBindings)) {
            ...
            for (const el of ic.namedBindings.elements) {
              // 不再跳过 type-only 导入，让 AST 扫描来判断是否真的被使用
   ...
   compiler.hooks.afterEmit.tapAsync('UnusedFilesPlugin', (compilation, callback) => {
     try {
      ...
      this.files.forEach(file => {
        ...
        // 但如果是 strictRuntimeUsage 模式，会通过 AST 分析实际使用情况
